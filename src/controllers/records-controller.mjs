...
export const listAllRecords = async (req, res) => {
    ...
    res.status(200).json({ success:true, statusCode: 200, blocks: records.chain, message: "All records found successfully" });
}

export const findById = async (req, res) => {
    const record = await new RecordRepository().findById(req.params.id);
    res.status(200).json({ success: true, statusCode: 200, data: record, message: "Record found successfully" });
}
